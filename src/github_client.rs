...
impl GithubClient {
    ...
    fn calc_most_contributing_percentage(mut data: Vec<Contributor>) -> Option<(String, f32)> {
        ...
        Some ((data[0].login.to_string(), (data[0].contributions as f32) / (all_contrubutions as f32)))
    }
    ...
    async fn get_top_contributor_percentage(&self, repo: Repository) -> Result<ContributionResult, ApiError> {
        ...
        let response = self.create_get_request_builder(&request_url)
            .query(&[
                ("sort", "contributions"),
                ("o", "desc"),
        ...
        if response.status() != StatusCode::OK{
            return Err(ApiError(format!("Error calling API - response code: {}", response.status())))
    }
        let data: Vec<Contributor> = response.json().await.map_err(|_x| ApiError("Error deserializing contributors data".to_owned()))?;
        let most_contribution = Self::calc_most_contributing_percentage(data).unwrap_or(("N/A".to_owned(),0 as f32));
    ...
    async fn get_repos_page<'a>(&self, lang: &str, i: i32) -> Result<PagedResult<Repository>,ApiError> {
        ...
        let data: PagedResult<Repository> = response.json().await.map_err(|_x| ApiError("Error deserializing repos data".to_owned()))?;
        Ok(data)
    }
    ...
    pub fn get_pages<'l>(&'l self, lang: &'l str) -> impl Stream<Item = Result<Repository,ApiError>> + 'l {
        stream::iter(0..)
            ...
            .map_ok(|x| stream::iter(x.items.into_iter().map(Ok))).try_flatten()
    }
